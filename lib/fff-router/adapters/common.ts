...
export function pathWithinScope(
	...
	return (
		candidatePath === request.within ||
		candidatePath.startsWith(request.within + path.sep)
	);
}
...
export function matchesExcludePaths(
	excludePaths: string[],
	relativePath: string,
): boolean {
...
export function filterItems(
	...
	return items
		...
		.filter((item) =>
			matchesExcludePaths(request.excludePaths, item.relativePath),
		)

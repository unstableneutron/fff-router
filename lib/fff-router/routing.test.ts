...
const config: RouterConfig = {
	allowlistedNonGitPrefixes: [
		{
			prefix: "/Users/thinh/.local/share/mise",
...
describe("deriveRoutingTarget", () => {
	...
	test("derives first-child non-git roots from the longest matching prefix", () => {
		...
		expect(result.value.persistenceRoot).toBe(
			"/Users/thinh/.local/share/mise/installs/npm-gitchamber",
		);
	...
	test("blocks exact allowlist prefixes in V1", () => {
		const result = deriveRoutingTarget({
			realPath: "/Users/thinh/.local/share/mise/installs",
			statType: "directory",
			gitRoot: null,
			config,
		});
	...
	test("rejects relative allowlist prefixes", () => {
		const result = deriveRoutingTarget({
			realPath: "/Users/thinh/.local/share/mise/installs/npm-gitchamber/latest",
			statType: "directory",
			gitRoot: null,
			config: {
				...config,
